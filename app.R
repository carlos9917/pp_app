...
# 2) INCLUDE THE EXPERIMENT NAME IN all_exp_names
#
# 06/2022
#========================================================================================#
...
#app_dir <- NULL
#app_dir <- "/media/cap/7fed51bd-a88e-4971-9656-d617655b6312/data/sources/pointver_app/visapp"
app_dir <- "visapp"
if (is.null(app_dir)){
  stop("Error: Please set app_dir to the visapp directory on your system")
}

#app_dir <- "/media/cap/7fed51bd-a88e-4971-9656-d617655b6312/data/sources/harp-verif/visualization/visapp"
# Point to where the image files are stored
img_dir <- NULL
if (is.null(img_dir)){
  # Look a default image dirctory in the app directory
  img_dir <- "sample_images" # Relative to the app_dir
}
...
# Ensemble
all_ens_ssum_scores      <- list("Spread Skill"              = "rmse~spread-lt",
                                 "Mean Bias RMSE"            = "mean_bias~rmse-lt",
                                 "CRPS"                      = "crps-lt",
                                 "Member Bias"               = "mbrbias-lt",
                                 "Member RMSE"               = "mbrrmse-lt",
                                 "Rank Histogram"            = "rank_histogram-NA",
                                 "Mean DailyVar"             = "dailyvar-vh",
                                 "Mean Bias STDV Timeseries" = "mean_bias~stde-vd",
...
all_ensctrl_ssum_scores  <- list("Bias RMSE"            = "bias~rmse-lt",
                                 "DailyVar"             = "dailyvar-vh",
                                 "Forecast Timeseries"  = "timeseries-vd",
                                 "Bias STDV Timeseries" = "bias~stde-vd",
...
all_det_ssum_scores      <- list("Bias RMSE"            = "bias~rmse-lt",
                                 "DailyVar"             = "dailyvar-vh",
                                 "Forecast Timeseries"  = "timeseries-vd",
                                 "Bias STDV Timeseries" = "bias~stde-vd",
                                 "Frequency Dist"       = "freqdist-NA",
...
ui <- shiny::tags$html(
  ...
  shiny::navbarPage( # Add the logo at the top
    title=div(img(src="cloud.png", #this part adds a logo. comment if unwanted
...
server <- function(input, output, session) {
  ...
  all_files <- shiny::reactive({
    list.files(path = data_dirname(),pattern="*.png")
  })
